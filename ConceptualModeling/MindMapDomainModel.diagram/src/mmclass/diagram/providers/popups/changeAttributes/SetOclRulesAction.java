...
public class SetOclRulesAction extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		eo = (Entity) selectedElement.resolveSemanticElement();
		EAttribute ea = MmclassPackage.eINSTANCE.getEntity_OclRules();	
		//call popup
		Entity node = (Entity) selectedElement.getNotationView().getElement();
		String value = node.getOclRules();
		
		final InputDialog inputDialog = new InputDialog(Display.getCurrent().getActiveShell(), "OCL Rule", "Enter Rules:", value, null) {
			/**
			...
			for(EditPart ep : allObjects) {
				if (ep instanceof EntityEditPart) {
					...
					if (l.getNodeText().equals(node.getNodeText())) {
						//refresh it
						ActiveDiagram.setOCL((ShapeNodeEditPart) ep, inputDialog.getValue());						
					}						
					...
					while (itE.hasNext()) {
						...
						if (l.getNodeText().equals(node.getNodeText())) {
							ActiveDiagram.setOCL((ShapeNodeEditPart) e2, inputDialog.getValue());
						}
					}													
				}
